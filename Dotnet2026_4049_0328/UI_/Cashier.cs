...
//        private BO.Customer? currentClient = null;
...
//                bl.Order.SearchSaleForProduct(pio, isPreferred);
...
//                var sales = bl.Order.AddProductToOrder(currentOrder, productId, quantity);
//                RefreshOrderGrid();
...
//            try
//            {
//                if (bl.Customer.Read(id)!=null)
//                {
//                    var c = bl.Customer.Read(id);
//                    currentClient = c;
...
namespace UI_
{
    public partial class Cashier : Form
    {
        // שימוש ב-Factory כדי לקבל את שכבת הלוגיקה
        IBl bl = Factory.Get();
        private BO.Order? currentOrder = null;
        private BO.Customer? currentClient = null;
        ...
        private void Cashier_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // שימוש ב-Property 'Product' (ולא IProduct)
                var products = bl.Product.ReadAll();
                comboProducts.DataSource = products.ToList();
                comboProducts.DisplayMember = "Name";
                comboProducts.ValueMember = "Id";
        ...
        private void UpdateDetails()
        {
            try
            {
                ...
                // תיקון: bl.Product.Read (ולא IProduct)
                var product = bl.Product.Read(productId);
                if (product == null)
                {
                    lblDetailName.Text = "Name: (not found)";
        ...
        private void addProductButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // תיקון: bl.Order.AddProductToOrder
                var sales = bl.Order.AddProductToOrder(currentOrder, productId, quantity);
                RefreshOrderGrid();
                MessageBox.Show($"Product added. Applied sales: {sales.Count}");
            }
        ...
        private void RefreshOrderGrid()
        {
            if (currentOrder == null || currentOrder.Products == null) return;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // תיקון: bl.Customer.Read
                var c = bl.Customer.Read(id);
                if (c != null)
                {
                    currentClient = c;
                    currentOrder = new BO.Order { IsPreferredClient = c.IsClubMember, Products = new List<ProductInOrder>() };
                    panelDetails.Visible = true;
                    panelOrder.Visible = true;
                    panelOpen.Visible = false;
                    RefreshOrderGrid();
                ...
                {
                    // לוגיקה ליצירת לקוח חדש אם לא נמצא (כפי שהיה לך)
                    HandleNewClient(id);
                }
            }
            catch (Exception ex)
        ...
        {
            var dr = MessageBox.Show($"Client ID {id} not found. Create new?", "Not Found", MessageBoxButtons.YesNo);
            if (dr == DialogResult.Yes)
            {
                using var form = new AddClientForm(bl, id);
                if (form.ShowDialog() == DialogResult.OK && form.CreatedClient != null)
                {
                    currentClient = form.CreatedClient;
                    currentOrder = new BO.Order { IsPreferredClient = currentClient.IsClubMember, Products = new List<ProductInOrder>() };
                    panelDetails.Visible = true;
                    panelOrder.Visible = true;
                    panelOpen.Visible = false;
                    RefreshOrderGrid();
